...
likelihood = function(param, dat, sigma = NA){
  ...
  a = briere(Temp, T0, Tm , c)
  a[a<=0] = 1e-4
  
  if(length(unique(dat$author))>1 ){
    
    LL = sum(  dexp(dat$bite_rate, rate = a, log = TRUE) )
  } else {
    LL = sum( log( dnorm(dat$bite_rate, mean = a, sd = sigma)) )
  }
...
MCMC_step = function(param, data, chain_cov, adapt, accCurrent){
  ...
  if(is.finite(prior_prop)){
    like_prop = likelihood( exp( param_prop ), data, sigma=NA)
...
name_dir = paste0("biterate_MCMC_chain", "_", date)
