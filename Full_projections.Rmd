...
#get_chains
mcmc_out=get_chains(path, burnin = 5e5, thin = 1)[, 1:29]
...
plot(Temp, temp_suitability(Temp ,apply(mcmc_out[,21:29], 2, median)), 
...
for (i in 1:n_samples){
  ind = sample(nrow(mcmc_out), 1)
  t_s = temp_suitability(Temp ,  mcmc_out[ind,21:29] )
...
### TEMP SUITABILITY ###
dat_full_temp = cbind(dat_full, temp_suitability(dat_full[,temp_type] , apply(mcmc_out[,21:29], 2 ,"median")) )
...
for(i in 1:n_samples){
  ...
  ### TEMP SUITABILITY ###
  dat_full_temp = cbind(dat_full, temp_suitability(dat_full[,temp_type] , mcmc_out[sample_ind,21:29])  )
  names(dat_full_temp)[ncol(dat_full_temp)] = "temp_suitability"
  envdat = launch_env_dat(filepath = NA, dat_full = dat_full_temp, c34 = c34)  
  ...
  ### preds
  Est_beta =  as.numeric( mcmc_out[sample_ind,1:20] )
  glmpreds_tmp = fun_calcPred( Est_beta ,x,type="response")
...
adjusted_params = c(exp(median(mcmc_out_f[,1])), 
                        apply(mcmc_out[,1:20], 2, median, na.rm = T), 
...
temp_param = apply(mcmc_out[,21:29], 2, median, na.rm = TRUE)
...
# then we take n_samples random samples of our posteriors to create our output.
adjusted_params = c(exp(median(mcmc_out_f[,1])), 
                    apply(mcmc_out[,1:20], 2, median, na.rm = TRUE), 
