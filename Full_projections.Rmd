...
knitr::opts_chunk$set(
  fig.path = "FullProjectionImagesNew/",
  dpi = 300
)
```
...
path = "GLM_tempsuit_MCMC_chain_20180910_hamlet"

#get_chains
mcmc_out=get_chains(path, burnin = 1, thin = 100)[, 1:28]
...
plot(Temp, temp_suitability(Temp ,apply(mcmc_out[,20:28], 2, median)), 
...
for (i in 1:n_samples){
  ind = sample(nrow(mcmc_out), 1)
  t_s = temp_suitability(Temp ,  mcmc_out[ind,20:28] )
...
for(i in 1:n_samples){
  ...
  ### TEMP SUITABILITY ###
  dat_full_temp = cbind(dat_full, temp_suitability(dat_full[,"ERAday.mean"] , mcmc_out[sample_ind,20:28])  )
  names(dat_full_temp)[ncol(dat_full_temp)] = "temp_suitability"
  envdat = launch_env_dat(filepath = NA, dat_full = dat_full_temp, c34 = c34)  
...
temp_param = apply(mcmc_out[,20:28], 2, median,na.rm = T)
...
for(i in 1:n_samples){
  ...
  adjusted_params = unlist(c(exp(mcmc_out_f[sample_ind_f,1]), #vac eff
                             mcmc_out[sample_ind,1:19] ,            #glm
                             exp(mcmc_out_f[sample_ind_f,c(2:41)]),  #foi
                             exp(mcmc_out_f[sample_ind_f,82]) ) )    #vc factor
  
  names(adjusted_params)[c(1,length(adjusted_params))] = c("vac_eff", "vc_factor_CMRs")
  
  temp_param = mcmc_out[sample_ind, 20:28]
