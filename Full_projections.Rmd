...
setwd("GLM_tempsuit_MCMC_chain_20180910_hamlet")
...
mcmc_out = as.data.frame( mcmc_out[1:nrow(mcmc_out),1:28] )

#thin
mcmc_out=mcmc_out[seq(1,nrow(mcmc_out),100),]
...
temp_suit_mat = NULL
for (i in 1:n_samples){
  t_s = temp_suitability(Temp , as.numeric( mcmc_out[sample(nrow(mcmc_out), 1),20:28]) )
...
for(i in 1:n_samples){
  ...
  temp_param = as.numeric( mcmc_out[sample_ind, 20:28])
