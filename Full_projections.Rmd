...
mycols =  (snapalette("BudapestCitadel",length(mybreaks)-1, type = "continuous"))
...
filepath = "Z:/MultiModelInference/multi_model_MCMC_chain_20180622" #get_latest_file(path = "Z:/MultiModelInference", pattern = "multi_model_MCMC_chain_")
#
...
outfoi = YFestimation::fun_calc_transmission_Africa(x, 
                                                    ii, 
                                                    seroout,
...
mybreaks = seq(-5, -1, length.out=101)
mycols =  ( snapalette( "Ipanema", length(mybreaks)-1, type = "continuous" ))
mm = match(shp1$adm0_adm1,envdat$dat$adm0_adm1)
vcols = findInterval(log10(Foi_runs),mybreaks)
...
image.plot(legend.only=TRUE, breaks=mybreaks, col=mycols, zlim=c(0,1), horizontal = TRUE,
               axis.args = list(at = c(-5:-1), labels =c("1e-05","1e-04", "0.001", "0.01", "0.1"), las =2),
               legend.mar = 3.5)
