...
```{r ERA, fig.cap="worldclim_temp_min. Black indicates that area is not in the YF endemic region."}

par(mar = 0*c(5.1,4.1,4.1,2.1))

mybreaks= seq(min(dat_full$worldclim_temp_min)-0.1, max(dat_full$worldclim_temp_min)+0.1, length.out=100)
mycols =   colorRampPalette(brewer.pal(9, "Oranges") )(length(mybreaks)-1)
mm = match(shp1$adm0_adm1, dat_full$adm0_adm1)
vcols = findInterval(dat_full$worldclim_temp_min,mybreaks)
