...
for(s in c(26,45,60,85)){
  for(y in c(2050, 2070)){
    ...
    mybreaks = seq(7.269308 , 34, length.out=100)

    mycols =   colorRampPalette(brewer.pal(9, "Oranges") )(length(mybreaks)-1)
    mm = match(shp1$adm0_adm1, dat$adm0_adm1)
    vcols = findInterval(dat$worldclim_temp_mid,mybreaks)
...
for(s in c(26,45,60,85)){
  for(y in c(2050, 2070)){
    ...
    mybreaks= seq(8.432164, 42.88494, length.out=100)
    mycols =   snapalette("Bouquet", n = length(mybreaks)-1, type = "continuous")
    mm = match(shp1$adm0_adm1, dat$adm0_adm1)
    vcols = findInterval(dat$worldclim_temp_range,mybreaks)
...
for(s in c(26,45,60,85)){
  for(y in c(2050, 2070)){
    ...
    mybreaks= seq(0, 350, length.out=100)
    mycols =   rev( snapalette("Venice", type = "continuous", length(mybreaks)-1)) #colorRampPalette(brewer.pal(9, "Blues") )(length(mybreaks)-1)
    mm = match(shp1$adm0_adm1, dat$adm0_adm1)
    vcols = findInterval(dat$worldclim_rainfall,mybreaks)
