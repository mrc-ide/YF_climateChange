---
title: "Median burden projections"
...
```{r median_infections_country, fig.height=12, fig.width=10}
...
df = df %>% mutate(Cases = Infections*P_severe,
                   Deaths = Infections*P_severe*P_severeDeath)

```

```{r median_burden, fig.height=12, fig.width=10}
...
```{r median_deaths_country, fig.height=12, fig.width=10}
...
```{r median_deaths_percap_country, fig.height=12, fig.width=10, eval = FALSE}

ggplot(df ) + 
  geom_line(aes(x = Year, y = Deaths/population, colour = scenario, group = scenario), stat='summary', fun.y=sum, size = 2) + 
  scale_colour_manual(values = snapalette("Camden", 5))+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  facet_wrap(~adm0)#, scales = "free_y")
