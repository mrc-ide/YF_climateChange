...
```{r plot_transmission_diff_2050, fig.height=10, fig.width=10, fig.cap="Force of infection, relative difference 2050.", echo=TRUE}
...
for(s in c("26", "45", "60", "85")){
    ...
    runs = (df_subs$runs - df_subs$runs_now) / df_subs$runs_now
...
```{r plot_transmission_diff_2070, fig.height=10, fig.width=10, fig.cap="Force of infection, relative difference 2070.", echo=TRUE}
...
for(s in c("26", "45", "60", "85")){
    ...
    mybreaks= seq(-6, 6, length.out=101)
    mm = match(shp1$adm0_adm1,envdat$dat$adm0_adm1)
    vcols = findInterval(runs, mybreaks)
