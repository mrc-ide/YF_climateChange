...
for(year in c(2050, 2070, "now")){
  ...
  for(scenario in c(26, 45, 60, 85)){
    ...
    runs_clim_change = rbind(runs_clim_change, cbind(runs, data.frame("year" = year, "scenario" = scenario)))
  }
}
...
```{r plot_transmission_all, fig.height=10, fig.width=7, fig.cap="Force of infection, %, scenarios.", echo=TRUE}
...
for(year in c(2050, 2070)){
  ...
  for(scenario in c(26, 45, 60, 85)){
    ...
    runs = runs_clim_change %>% filter(year == year & scenario == scenario)
    ...
    plot(shp0, xlim=c(-15,50),ylim=c(-20,30), main = paste0(year, "_", scenario))
    mm0 = match(shp0$ISO,Countries$c34) #
    plot(shp0[is.na(mm0),],col="black",add=T)
    plot(shp1[!is.na(mm),],col=colours[vcols], xlim=c(-15,50),ylim=c(-20,30) , lty=0, add=T)
...
```{r plot_transmission_diff_2050, fig.height=10, fig.width=7, fig.cap="Force of infection, relative difference 2050.", echo=TRUE}

colours = (snapalette("Fjord", 100, type = "continuous"))

trans_diff = data.frame(scenario = filter(runs_clim_change, year == 2050)$scenario)

trans_diff$year_2050 = (filter(runs_clim_change, year == 2050)$runs - filter(runs_clim_change, year == "now")$runs)/filter(runs_clim_change, year == "now")$runs
trans_diff$year_2070 = (filter(runs_clim_change, year == 2070)$runs - filter(runs_clim_change, year == "now")$runs)/filter(runs_clim_change, year == "now")$runs 

par(mfrow = c(2, 2), mar = c(2,2,2,2))

for(scenario in c(26, 45, 60, 85)){

  runs = filter(trans_diff, scenario == scenario)$year_2050
    
    mybreaks= seq(-1.1, 1.1, length.out=101)
    mm = match(shp1$adm0_adm1,envdat$dat$adm0_adm1)
    vcols = findInterval(runs, mybreaks)
    
    plot(shp0, xlim=c(-15,50),ylim=c(-20,30), main = paste0( scenario))
    mm0 = match(shp0$ISO, Countries$c34) #
    plot(shp0[is.na(mm0),],col="black",add=T)
    plot(shp1[!is.na(mm),],col=colours[vcols], xlim=c(-15,50),ylim=c(-20,30) , lty=0, add=T)
...
```{r plot_transmission_diff_2070, fig.height=10, fig.width=7, fig.cap="Force of infection, relative difference 2070.", echo=TRUE}
...
for(scenario in c(26, 45, 60, 85)){
  ...
  runs = filter(trans_diff, scenario == scenario)$year_2070
