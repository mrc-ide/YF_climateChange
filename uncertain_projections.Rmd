...
knitr::opts_chunk$set(
  fig.path = paste0("Uncertain_projections_stop_", Sys.Date(), "/"),
  dpi = 300,
  dev = c('png')
)
```
...
ggplot( filter(inf_df, Year %in% c(2050, 2070) )) + 
  geom_density_ridges(scale = 2, 
                      alpha = 0.5,
                      aes(x = Deaths, y = reorder(adm0, Deaths), fill = scenario, colour = scenario),
                      alpha = 0.7) + 
                     #jittered_points = TRUE) + 
                     #position =  "raincloud",
                      # point_size = 3, alpha = 0.7, point_alpha = 0.1) + 
  scale_fill_manual(values = c(snapalette(snapal)[1:4], "white"))+
  scale_color_manual(values = c(snapalette(snapal)[1:4], "black"))+
  theme_ridges()+
  xlab("Deaths") + 
  ylab("Country")+
  scale_y_discrete(expand = c(0.01, 0)) +
...
ggplot( filter(inf_df, Year %in% c(2050, 2070) )) + 
    geom_density_ridges(scale = 2, 
                      aes(x = Deaths/total_pop, y = reorder(adm0, Deaths/total_pop), fill = scenario, colour = scenario),
                      alpha = 0.7) + 
  scale_fill_manual(values = c(snapalette(snapal)[1:4], "white"))+
  scale_color_manual(values = c(snapalette(snapal)[1:4], "black"))+
  theme_ridges()+
...
```{r deaths_percap_country3_2050, fig.height=12, fig.width=10, fig.cap="Deaths per capita per country."}

tmp_df = filter(inf_df, Year %in% c(2050) )

mu = tmp_df %>% group_by(scenario, adm0) %>% summarise(group_median = median(Deaths/total_pop))

ggplot( tmp_df) + 
  geom_density(aes( x = Deaths/total_pop,  fill = scenario), alpha = 0.7) + 
  scale_fill_manual(values = c(snapalette(snapal)[1:4], "white"))+
  scale_color_manual(values = c(snapalette(snapal)[1:4], "white"))+
  theme_ridges()+
  xlab("Deaths per capita") + 
  ylab("Country")+
  #scale_y_discrete(expand = c(0.01, 0)) +
  facet_wrap(~adm0, scales = "free_x")+
  scale_x_log10()+
  theme(text = element_text(size = 20)) +
  geom_vline(data = mu, aes(xintercept = group_median, colour = scenario))
```

```{r deaths_percap_country3_2070, fig.height=12, fig.width=10, fig.cap="Deaths per capita per country."}

tmp_df = filter(inf_df, Year %in% c(2070) )
...
for(s in 1:4){
  for(y in c(2050, 2070)){
    ...
    tmp_df = data.frame(scenario = scen_vec[s],
                        ...
                        proportion_pos_ci = paste0("[",round(bout_ci_calc[1], digits = 4)
                                                   , ", ", 
                                                   round(bout_ci_calc[3], digits = 4) , "]"))
