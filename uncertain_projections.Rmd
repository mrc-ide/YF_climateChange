...
```{r deaths_percap_country_2050, fig.height=12, fig.width=10, fig.cap="Deaths per capita per country."}
ggplot( filter(inf_df, Year %in% c(2050) )) + 
    geom_density_ridges(scale = 3, 
...
tmp = inf_df %>% filter( scenario == "current")
...
ggplot( filter(inf_df, scenario != "current")) + 
  geom_density_ridges(scale = 0.9, 
