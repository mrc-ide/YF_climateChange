...
tmp$scenario = "now"
tmp = tmp %>% dplyr::rename(FOI_now = FOI) %>% select("adm0", "FOI_now", "sample")
...
ggplot( filter( df, year %in% c(2050, 2070) & scenario %in% c(26, 85) )) + 
  geom_violin(aes(x = year, y = percent_diff, fill = scenario), position="dodge", draw_quantiles = 0.5) + 
   ylim(-200, 500) +
  facet_wrap(adm0~.) +
  scale_fill_manual(values = snapalette(snapal)[c(1,4,5)])+
...
```{r infections_scenario_subset_country, fig.cap="Infections.", fig.height = 12, fig.width = 10}
tmp = filter(inf_df, 
             Year %in% c(2018, 2050, 2070) & 
                           scenario %in% c("26", "85", "now"))
...
P_severe_runs = rbeta(1000,6.367309,44.60736)
P_death_runs = rbeta(1000,16.43466, 18.49048)

burden_df = inf_df %>% mutate(Cases = Infections * P_severe_runs,
                              Deaths = Cases * P_death_runs)

```

```{r deaths_scenario_country, fig.cap="Deaths.", fig.height = 12, fig.width = 10}
tmp = filter(burden_df, 
             Year %in% c(2018, 2050, 2070) )
...
```{r deaths_scenario_subset_country, fig.cap="Deaths.", fig.height = 12, fig.width = 10}
tmp = filter(burden_df, 
             Year %in% c(2018, 2050, 2070) & 
                           scenario %in% c("26", "85", "now"))
...
pop_all = pop_all %>% dplyr::rename(adm0 = country_code, Year = year) %>% mutate(Year = as.character(Year))

burden_df = left_join(burden_df, pop_all, by = c("adm0", "Year"))
...
```{r deaths_percap_country_subset, fig.height=12, fig.width=10, fig.cap="Deaths per capita per country."}
...
tmp = filter(burden_df, scenario == "now")
tmp %<>% dplyr::rename(Deaths_now = Deaths) %>% select("adm0", "Year", "Deaths_now", "sample")

df = left_join(burden_df, tmp, by = c("adm0", "Year", "sample"))

tmp = filter(df, 
             Year %in% c(2018, 2050, 2070) )

ggplot(tmp , aes(x = Year, y = ((Deaths - Deaths_now)/ Deaths_now)*100, fill = scenario)) + 
  geom_violin( position="dodge", draw_quantiles = 0.5) +
  scale_fill_manual(values = snapalette(snapal))+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
```{r relative_deaths_country_subset, fig.height=12, fig.width=10, fig.cap="Proportion of deaths relative to baseline per country."}
...
```{r relative_deaths_subset, fig.height=12, fig.width=10, fig.cap="Proportion of deaths relative to baseline."}
...
```{r deaths_table, fig.cap="Deaths per year in each scenario"}

df2 = df %>% mutate(Deaths_change = ((df$Deaths - df$Deaths_now)/ df$Deaths_now)*100)
...
out_df = df2 %>% 
  group_by( Year, scenario) %>% 
  summarise_at(vars(Deaths_change), funs(!!!p_funs)) %>% 
...
tmp %<>% mutate(WE = ifelse(adm0 %in% west, "West", "East and central"))
...
```{r change_deaths_by_region_subset, fig.cap = "Change in deaths per region"}
...
```{r deaths_by_region_subset}

ggplot(tmp , aes(x = Year, y = Deaths, fill = scenario)) + 
  geom_violin( position="dodge", draw_quantiles = 0.5) +
  scale_fill_manual(values = snapalette(snapal)[c(1,4,5)])+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
