...
```{r difference_by_area2, fig.width=10, fig.height=8}
...
```{r deaths_percap_by_region2, out.width="1\\linewidth"}


ggplot(inf_df, aes(x = Deaths_percap, y =  Year, fill = scenario)) + 
  geom_density_ridges(alpha = 0.6 , scale = 1) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_fill_manual(values = c(snapalette(snapal)[1:4], "white")) +
