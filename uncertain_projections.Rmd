...
```{r transmission_by_area , eval = FALSE}
ggplot(tp) + 
  geom_violin(aes(fill = scenario, y = FOI, x = year), position="dodge", draw_quantiles = 0.5) + 
  facet_wrap(WE~.)+
...
```{r difference_by_country, fig.height = 12, fig.width = 10, eval = FALSE}
...
for(s in 1:4){
  for(y in c(2050, 2070)){
    ...
    g = ggplot( filter( transmission_proj, year %in% y & scenario %in% scen_vec[s] )) + 
      geom_density_ridges(scale = 2, aes(x = percent_diff, y = reorder(adm0, percent_diff) ), 
                          fill = col,
                          alpha = 0.5) + 
      theme_ridges()+
...
```{r difference_by_area,  eval = FALSE}
...
```{r difference_by_area2, fig.width=10, fig.height=8, eval = FALSE}


ggplot( filter( transmission_proj, year %in% c(2050, 2070) )) + 
  geom_density_ridges(alpha = 0.6 , scale = 0.9, aes(x = percent_diff, y = year, fill = scenario)) + 
  facet_wrap(WE~.) +
  scale_fill_manual(values = c(snapalette(snapal)[1:4], "white"))+
  theme_ridges()+
  xlab("Change in force of infection (%)") + 
...
kableExtra::kable(out_df, caption = "FOI per year in each scenario")
...
```{r deaths_scenario_country, fig.cap="Deaths.", fig.height = 12, fig.width = 10, eval = FALSE}
...
```{r deaths_percap_country, fig.height=12, fig.width=10, fig.cap="Deaths per capita per country.", eval = FALSE}
...
```{r relative_deaths_country, fig.height=12, fig.width=10, fig.cap="Proportion of deaths relative to baseline per country.", eval = FALSE}
...
```{r relative_deaths, fig.height=12, fig.width=10, fig.cap="Proportion of deaths relative to baseline.", eval = FALSE}

ggplot(inf_df, aes(x = Year, y = relative_deaths, fill = scenario)) + 
  geom_violin( position="dodge", draw_quantiles = 0.5) +
  scale_fill_manual(values = c(snapalette(snapal)[1:4], "white"))+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
```{r percentage_change_deaths_per_scenario_table, eval = FALSE}
...
```{r change_deaths_by_region, fig.cap = "Change in deaths per region", eval = FALSE}
...
```{r deaths_percap_by_region, eval = FALSE}

ggplot(inf_df, aes(x = Year, y = Deaths_percap, fill = scenario)) + 
  geom_boxplot( position="dodge", draw_quantiles = 0.5) +
  scale_fill_manual(values = c(snapalette(snapal)[1:4], "white"))+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ylab("Deaths per capita")+
